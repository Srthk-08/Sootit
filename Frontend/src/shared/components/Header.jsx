...
const AppHeader = () => {
  ...
  return (
    <motion.header 
      initial={{ y: -40 }}
      animate={{ y: 0 }}
      className={`sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-black/[0.03] py-2 px-3 flex items-center justify-between h-16 ${isAdmin ? 'bg-neutral-900/10 border-neutral-900/10' : ''} font-inter`}
    >
      <div className="flex items-center gap-3 group">
        <div className={`h-10 w-10 rounded-2xl flex items-center justify-center font-black text-white text-xs shadow-lg ${isVendor ? 'bg-indigo-600 shadow-indigo-600/20' : isAdmin ? 'bg-neutral-900 shadow-black/20' : 'bg-red-600 shadow-red-600/20'}`}>
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1.5 cursor-pointer">
            <span className="text-sm font-black text-neutral-900 leading-none tracking-tight">
              {isAdmin ? 'DeepMind' : isVendor ? 'Arjun Dev' : 'Connaught Place'}
            </span>
            <ChevronDown size={12} className="text-red-600" strokeWidth={3} />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {!isAdmin && !isVendor && (
          <div className="bg-red-50/80 border border-red-100/50 px-3 py-1.5 rounded-2xl flex items-center gap-2 shadow-sm scale-90 origin-right transition-transform active:scale-95">
            <Wallet size={14} className="text-red-600" strokeWidth={2.5} />
            <span className="text-xs font-black text-red-700 leading-none tracking-tighter">₹{userData?.wallet?.toFixed(1) || "0.0"}</span>
          </div>
        )}
